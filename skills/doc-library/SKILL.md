...
Neither is strictly safer; they swap a recurring filtering cost for a rarer migration cost. Status-in-place degrades more gracefully under loose discipline (a missed promotion leaves a visible `proposed`, not a hole), so it's the reasonable default for most repos; reach for location separation mainly when proposed decisions are sensitive and the repo is public. Either way, `doc-audit` is the backstop — it promotes on landing and flags both failure directions. The `status` field then carries the rest of the lifecycle: `accepted → superseded`/`deprecated`.
